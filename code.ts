...
function hslToRgbFigma(h: number, s: number, l: number) {
    let r, g, b;
    if (s === 0) {
        r = g = b = l; 
    } else {
...
function generateColorScale(baseHex: string) {
    ...
    const scale: Record<number, {h: number, s: number, l: number}> = {
        50:  { h, s, l: 0.96 },
        100: { h, s, l: 0.90 },
        200: { h, s, l: 0.80 },
        300: { h, s, l: 0.70 },
...
async function runGenerator(colors: Record<string, string>, font: string) {
    ...
    let textStyle = figma.getLocalPaintStyles().find(s => s.name === `Text/Default`);
    if (!textStyle) {
        textStyle = figma.createPaintStyle();
        textStyle.name = `Text/Default`;
    }
    textStyle.paints = [{ type: 'SOLID', color: { r: 0.1, g: 0.1, b: 0.1 } }];
    
    let surfaceStyle = figma.getLocalPaintStyles().find(s => s.name === `Surface/Default`);
    if (!surfaceStyle) {
        surfaceStyle = figma.createPaintStyle();
        surfaceStyle.name = `Surface/Default`;
    }
    surfaceStyle.paints = [{ type: 'SOLID', color: { r: 1, g: 1, b: 1 } }];

    font = font.split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' '); 
    const s = 16;
